...
<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"0 30px","background":"#FFFFFF"}'>
		<!-- 列表页 -->
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","position":"relative","borderRadius":"24px","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">订单编号</label>
					...
					<el-button class="search" type="success" @click="ordertypeClick()">
						<span class="icon iconfont icon-fangdajing01" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","display":"none","height":"40px"}'></span>
						{{ordertype==1?'自提':'配送'}}订单
					</el-button>
				</el-row>

				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"20px 0 0px  -4px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('orders'+'/'+orderStatus,'新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 0px","fontSize":"16px","color":"#FFFFFF","height":"auto"}'></span>
						新建
					</el-button>
					<el-button class="del" v-if="isAuth('orders'+'/'+orderStatus,'删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#FFFFFF","height":"auto"}'></span>
						删除
					</el-button>
					...
					<download-excel v-if="isAuth('orders'+'/'+orderStatus,'导出')" class="export-excel-wrapper" :fetch="getAllList" :fields="json_fields" name="订单.xls">
						...
						<el-button class="btn18" type="success">
							<span class="icon iconfont icon-xiazai7" :style='{"margin":"0 0px","fontSize":"16px","color":"#000000","height":"auto"}'></span>
							导出
						</el-button>
					</download-excel>
					<el-button class="btn18" v-if="isAuth('orders'+'/'+orderStatus,'发货')&&ordertype==1" type="success" @click="updateHandler(null)">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 0px","fontSize":"16px","color":"#000000","height":"auto"}'></span>
						批量发货
					</el-button>
					<el-button class="btn18" v-if="isAuth('orders'+'/'+orderStatus,'确认收货')&&ordertype==1" @click="updateHandler2(null)">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 0px","fontSize":"16px","color":"#000000","height":"auto"}'></span>
						批量收货
					</el-button>
					<el-button class="btn18" v-if="isAuth('orders'+'/'+orderStatus,'核销')&&ordertype==2" type="success" @click="writeOff(null)">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 0px","fontSize":"16px","color":"#000000","height":"auto"}'></span>
						批量核销
					</el-button>
				</el-row>
			</el-form>
			<div :style='{"border":"0px solid #e9eafc","width":"100%","padding":" 0px 15px 20px 0","boxShadow":"none","borderRadius":"10px","background":"#fff"}'>
				<el-table class="tables"
					:stripe='false'
					:style='{"width":"100%","borderColor":"#999999","borderRadius":"10px","borderStyle":"solid","borderWidth":"0px 0 0 0px","background":"linear-gradient( 178deg, #C6D8FF 0%, #FFFFFF 100%)"}' 
					:border='false'
					...
					<el-table-column :resizable='true' :sortable='true'
												prop="address"
						label="地址" v-if="ordertype==1">
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
												prop="tel"
						label="电话" v-if="ordertype==1">
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
												prop="consignee"
						label="收货人" v-if="ordertype==1">
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
												prop="remark"
						label="备注">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'物流')&&scope.row.id&&scope.row.ordertype==1" type="success" @click="logisticsUpdate(scope.row.id)">
								<span class="icon iconfont icon-wuliu8" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								物流
							</el-button>
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'发货')&&scope.row.id&&scope.row.ordertype==1" type="success" @click="updateHandler(scope.row)">
								<span class="icon iconfont icon-fahuo16" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								发货
							</el-button>
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'确认收货')&&scope.row.id&&scope.row.ordertype==1" type="success" @click="updateHandler2(scope.row)">
								<span class="icon iconfont icon-queren12" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								确认收货
							</el-button>
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'核销')&&scope.row.id&&scope.row.ordertype==2" type="success" @click="writeOff(scope.row)">
								<span class="icon iconfont icon-hexiao15" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								核销
							</el-button>
		...
		<template v-if="statType">
			<div class="statShow8">
				<div class="echarts1" v-if="isAuth('orders'+'/已完成','商品销量')">
					<div id="buynumberChart1" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts2" v-if="isAuth('orders'+'/已完成','商品销额')">
					<div id="totalChart2" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts3" v-if="isAuth('orders'+'/已完成','日销量')">
					<div id="buynumberChart3" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts4" v-if="isAuth('orders'+'/已完成','月销量')">
					<div id="buynumberChart4" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts5" v-if="isAuth('orders'+'/已完成','年销量')">
					<div id="buynumberChart5" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts6" v-if="isAuth('orders'+'/已完成','日销额')">
					<div id="totalChart6" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts7" v-if="isAuth('orders'+'/已完成','月销额')">
					<div id="totalChart7" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts8" v-if="isAuth('orders'+'/已完成','年销额')">
					<div id="totalChart8" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
//导出excel
		...
		methods: {
			statChartClick() {
				if(this.isAuth('orders'+'/已完成','商品销量')) {
					this.chartDialogShow1()
				}
				if(this.isAuth('orders'+'/已完成','商品销额')) {
					this.chartDialogShow2()
				}
				if(this.isAuth('orders'+'/已完成','日销量')) {
					this.chartDialogShow3()
				}
				if(this.isAuth('orders'+'/已完成','月销量')) {
					this.chartDialogShow4()
				}
				if(this.isAuth('orders'+'/已完成','年销量')) {
					this.chartDialogShow5()
				}
				if(this.isAuth('orders'+'/已完成','日销额')) {
					this.chartDialogShow6()
				}
				if(this.isAuth('orders'+'/已完成','月销额')) {
					this.chartDialogShow7()
				}
				if(this.isAuth('orders'+'/已完成','年销额')) {
					this.chartDialogShow8()
				}
			},
			queryChange(arr){
				for(let x in arr) {
			...
			chartDialog4() {
				this.$nextTick(()=>{
					var buynumberChart4 = echarts.init(document.getElementById("buynumberChart4"),'macarons');
					let params = {}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							buynumberChart4.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								buynumberChart4.resize();
							};
						}
					})
				})
			...
			chartDialog7() {
				this.$nextTick(()=>{
					var totalChart7 = echarts.init(document.getElementById("totalChart7"),'macarons');
					let params = {}
					this.$http({
