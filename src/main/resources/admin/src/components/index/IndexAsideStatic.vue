			...
			if(menus) {
				...
				if(item.roleName == this.$storage.get('role')) {
  					item.backMenu.forEach(item1=>{
						if(item1.child&&item1.child.length>1 &&item1.menu != '订单管理') {
							item1.child.forEach(item2=>{
								if(item2.tableName=='orders') {
									if(
										this.isAuth('orders'+'/已完成','商品销量')
										||this.isAuth('orders'+'/已完成','商品销额')
										||this.isAuth('orders'+'/已完成','日销量')
										||this.isAuth('orders'+'/已完成','月销量')
										||this.isAuth('orders'+'/已完成','年销量')
										||this.isAuth('orders'+'/已完成','日销额')
										||this.isAuth('orders'+'/已完成','月销额')
										||this.isAuth('orders'+'/已完成','年销额')
									) {
										item1.child.push({
											allButtons: ['查看'],
											buttons: ['查看'],
											menu: '订单统计',
											tableName: 'ordersstat',
										})
									}
								}
							})
						}else if(item1.child&&item1.child.length==1 ||item1.menu == '订单管理') {
							if(item1.menu == '订单管理') {
								if(
									this.isAuth('orders'+'/已完成','商品销量')
									||this.isAuth('orders'+'/已完成','商品销额')
