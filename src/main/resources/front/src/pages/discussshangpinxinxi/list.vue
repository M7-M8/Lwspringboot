				...
				<el-table class="tables" :stripe='false'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="table-del" type="success" @click.native="delClick(scope.row)" v-if="btnAuth('shangpinxinxi','删除评论')">
								<span class="icon iconfont icon-shanchu6"></span>
								删除
							</el-button>
							<el-button class="table-btn1" type="success" @click.native="discussClick(scope.row)" v-if="btnAuth('discussshangpinxinxi','查看评论')">
	...
	export default {
		...
		methods: {
			...
			discussClick (row){
				...
				this.$router.push({path: '/index/discussdiscussshangpinxinxi', query: params});
			},
			editClick (row){
				this.$router.push(`/index/discussshangpinxinxiAdd?type=edit&&id=${row.id}`);
			},
			async delClick (row){
				await this.$confirm('是否删除此评论？') .then(_ => {
					this.$http.post('discussshangpinxinxi/delete', [row.id]).then(async res => {
						if (res.data.code == 0) {
							await this.$http.get('shangpinxinxi/info/' + row.refid).then(async rs=>{
								rs.data.data.discussnum--
								await this.$http.post('shangpinxinxi/update',rs.data.data).then(rs2=>{})
							})
							this.$message({
								message: "操作成功",
								type: "success",
			...
			discussistopChange(e,row){
				this.$http.post('discussshangpinxinxi/update',row).then(res=>{})
			},
		}
	}
</script>
