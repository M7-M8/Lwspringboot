<template>
	<div :style='{"width":"100%","padding":"20px 10% 40px","margin":"10px auto","position":"relative","background":"none"}'>
		<div class="section-title" :style='{"padding":" 0 0 0 10px","borderColor":"#ff6d6d","margin":"20px 0 10px 0","color":"#333","borderWidth":"0 0 0 6px","width":"auto","lineHeight":"1","fontSize":"32px","borderStyle":"solid"}'>确认{{totalPrice>0?'下单':'预订'}}</div>
		<div v-if="seat!=1" style="margin: 10px 0;width: 100%;display: flex;justify-content: center;">
			<el-radio-group v-model="ordertype">
				<el-radio-button :label="1">配送</el-radio-button>
				<el-radio-button :label="2">自提</el-radio-button>
			</el-radio-group>
		</div>
		<el-descriptions v-if="seat!=1 &&ordertype==1" title="选择收货地址"></el-descriptions>
		<el-table v-if="seat!=1 &&ordertype==1"
			:data="addressList"
		...
		<div v-if="seat!=1 &&ordertype==1" style="width:100%;display:flex;align-items:center;justify-content:center;margin:10px 0">
...
export default {
	...
	methods: {
		...
		beforePay(){
			if (this.radio == -1 &&this.ordertype==1) {
				this.$message({
					message: '请选择收货地址',
					type: 'error',
					duration: 1500
					...
					// 更新库存信息
					 this.$http.post(`${item.tablename}/update`, data).then( res => {
						...
						let order = {
							...
							address: this.ordertype==1?this.addressList[this.radio].address:'',
							tel: this.ordertype==1?this.addressList[this.radio].phone:'',
							consignee: this.ordertype==1?this.addressList[this.radio].name:'',
							remark: this.remark,
							status: '未支付',
							sfsh: '',
