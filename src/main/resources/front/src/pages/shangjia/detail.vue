			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.dianpuming}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						...
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('shangjia','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						if(res.data.data.dianpuming){
							this.fname = res.data.data.dianpuming
						}
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chaptershangjia', query: params});
			},
			// 获取uuid
			getUUID() {
		...
		.attr {
			...
			.info {
				...
				.item {
					...
					.uploadBtn:hover {
						border: 1px solid rgba(75, 223, 201,.5);
