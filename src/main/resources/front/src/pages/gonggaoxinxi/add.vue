...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="公告封面" v-if="type!='cross' || (type=='cross' && !ro.gonggaofengmian)" prop="gonggaofengmian">
				<file-upload
					tip="点击上传公告封面"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="公告封面" prop="gonggaofengmian">
				<img v-if="ruleForm.gonggaofengmian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.gonggaofengmian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.gonggaofengmian.split(',')" :src="baseUrl+item">
			</el-form-item>
			<el-form-item class="add-item" label="公告简介" prop="gonggaojianjie">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.gonggaojianjie"
					placeholder="公告简介"
...
<script>
	export default {
		...
		methods: {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='gonggaofengmian'){
							this.ruleForm.gonggaofengmian = obj[o]?obj[o].split(",")[0]:'';
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`gonggaoxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
