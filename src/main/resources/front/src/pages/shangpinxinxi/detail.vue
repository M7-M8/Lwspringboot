		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item item-price" v-if="detail.price">
						...
						<div class="text"><span style="font-size: 12px">￥</span>{{detail.price}}</div>
					</div>
					<div class="item item-price" v-if="detail.groupprice&&detail.groupprice>0&&detail.grouppeople>1">
						<div class="lable">团购价</div>
						<div class="text"><span style="font-size: 12px">￥</span>{{Number(detail.groupprice).toFixed(2)}}</div>
					</div>
					<div class="item" v-if="detail.grouppeople>1">
						<div class="lable">成团人数</div>
						<div class="text bold">{{detail.grouppeople}}人(仅差{{detail.grouppeople-detail.curpeople}}人成团)</div>
					</div>
					...
					<div class="item">
						...
						<div class="text  link" @click="merchantClick('dianpuming')" >{{detail.dianpuming}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						...
						<el-button class="buyBtn" v-if="!detail.curpeople&&detail.grouppeople>1" type="warning" size="small" @click="groupBuyTap">我要开团</el-button>
						<el-button class="buyBtn" v-if="detail.curpeople&&detail.grouppeople>1" type="warning" size="small" @click="groupBuyTap">去参团</el-button>
					</div>
					<div class="btn_box">
						...
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('shangpinxinxi','章节管理')">章节管理</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('shangpinxinxi','查看评论')">查看评论</el-button>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="商品描述" name="1">
					<div class="ql-snow ql-editor" v-html="detail.shangpinmiaoshu"></div>
				</el-tab-pane>
						...
						<div class="comment-item" v-for="item in infoList" :key="item.id" @mouseenter="discussEnter(item.id)" :id="'discuss' + item.id"
							...
							<div class="comment-content-box">
								...
								<div class="comment-btn">
									<!-- <el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px","outline":"none","color":"#fff","borderRadius":"4px","background":"#ff6d6d","width":"auto","lineHeight":"34px","fontSize":"14px","height":"34px"}'>回复</el-button> -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			merchantClick(name){
				if(this.detail.isanon=='是'&&this.detail.dianpuming != this.username) {
					return false
				}
				this.$http.get('shangjia/query',{params: {
					dianpuming: this.detail.dianpuming
				}}).then(rs=>{
					let params = {
						id: rs.data.data.id
					}
			...
			async thumbsupOrCrazily(type) {
				...
				await this.$http.post('storeup/add', this.storeupParams).then(res => {
					if (res.data.code == 0) {
						...
						this.$http.post('shangpinxinxi/update', detail).then(res => {
							this.detail = detail
						});
						this.getThumbsupOrCrazilyStatus();
						this.$message({
			...
			addComments(content=null,type=1) {
				...
				this.$http.post('discussshangpinxinxi/add', data).then(rs2 => {
					if (rs2.data.code == 0) {
						if(type==1) {
							this.form.content = '';
						}
			...
			addCart() {
				...
				this.cart.dianpuming = this.detail.dianpuming;
			...
			buyNow() {
				...
				groupBuyTap() {
					...
					localStorage.setItem('orderGoods', JSON.stringify([{
						...
						discountprice: this.detail.groupprice,
					}]));
			...
			onshelvesClick(){
				this.$confirm(`是否${this.detail.onshelves==1?'下架':'上架'}？`).then(async _ => {
					...
					await this.$http.post('shangpinxinxi/update',this.detail).then(rs=>{
